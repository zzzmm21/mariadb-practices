...
-- 예제 1) 현재, 직원의 이름과 직책을 출력하되 여성 엔지니어(engineer) 만 출력하세요.
...
-- 1) Natural Join
-- 두 테이블에 이름이 같은 공통 컬럼이 있으면 조인 조건을 명시 하지 않아도 암묵적으로 조인이 됨
...
-- 실습문제 1 : 현재 회사 생활을 반영한 직원별 근무 부서를 사번, 직원 전체 이름 ,근무 부서 형태로 출력
